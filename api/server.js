...
app.post("/bulkExport", async (req, res) => {
  ...
  // if the query is valid, add it to the queue
  // cannot be subject to mongodb injection, as
  // the query is generated by us, the user doesn't
  // directly create the query
  const data = {
    internalQuery: query,
    isCompleted: false,
    downloadLink: null,
    internalPath: null,
    batchJobType: "CSV/BULK",
    ipAddress: req.ip,
    requestedAt: Math.floor(Date.now() / 1000),
  };
  const result = await conn.insertOne(data);
